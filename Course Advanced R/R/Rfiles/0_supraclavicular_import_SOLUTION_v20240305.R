  ...
  # Hint: look how dataset struma was prepared
   ...
   # change data type and add labels to factors
      dt_supraclavicular <-  
        ...
        # change all categorical variables to factors and add labels
        mutate(group = factor(group, 
                              levels = c(1, 2),
                              labels = c("mixture", "sequential")),
               gender = factor(gender, 
                               levels = c(0, 1), 
                               labels = c("female", "male")),
               nerve_block_censor = factor(nerve_block_censor,
                                           levels = c(0, 1),
                                           labels = c("achieved nerve block",
                                                      "censored (failed block)")),
               med_censor = factor(med_censor,
                                   levels = c(0, 1),
                                   labels = c("received medication",
                                              "censored (no medication)"))) %>% 
        # apply same factor levels to several parameter
        mutate(across(c(vps_rest, vps_movement), ~ factor(.x, 
                                                          levels = c(0:10),
                                                          # labels = c("0 - no pain", 1:9, "10 - max pain")
                                                          labels = c(0:10)
        )))
