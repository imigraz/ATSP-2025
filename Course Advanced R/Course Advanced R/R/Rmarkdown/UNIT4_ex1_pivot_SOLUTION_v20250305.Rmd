---
title: "UNIT4 - Exercise 1 (SOLUTION)"
...
ggplot(data = dt_study_long,
       aes(x = measurement, y = systbp, colour = patient, group = patient)) +
  geom_line() +
  geom_point() +
  theme_bw()
```
...
dt2 <-
   dt_study2 %>% 
    select(patient, age, date1, date2, date3) %>% 
    pivot_longer(cols = c(date1, date2, date3),
                 names_to = "measurement",
                 values_to = "dates") %>% 
   mutate(measurement = str_remove(string = measurement, pattern = "date"))
...
ggplot(data = dt_study2_long,
       aes(x = dates, y = systbp, colour = patient, group = patient)) +
  geom_line() +
  geom_point() +
  ylim(c(75, 125)) +
  theme_bw()
...
#      we will need only: PatId, bloodgrp, weight, weight_1year
 ...
 dt_study_long <-
  dt_struma %>% 
    select(PatId, bloodgrp, weight, weight_1year) %>% 
    pivot_longer(cols = c(weight, weight_1year),
                 names_to = "measurement",
                 values_to = "weight")
```
...
```{r,  echo = TRUE, eval = TRUE}
# plot weight and measurement with ggplot and geom_line()
  # hint: you need to have in aes(..., group = patient)
  #       try also to have colour related to parameter bloodgroup

ggplot(data = dt_study_long,
       aes(x = measurement, y = weight, colour = bloodgrp, group = PatId)) +
  geom_line() +
  geom_point() +
  theme_bw()
```